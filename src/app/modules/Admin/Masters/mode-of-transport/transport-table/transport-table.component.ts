...
export class TransportTableComponent implements OnInit {
  ...
  // constructor
    ...
    getModeOfTransportList() {
      ...
      this.masterSer.onPostMethodSync(`${this.ApiUrl1}master/modeOfTransportMaster/list`, ReqObj).subscribe(
        (data: any) => {
          console.log(data);
          
          if(data.Message == 'Success') {
            this.columnHeader = [
              {key: 'ModeOfTransportId', display: 'Mode Of Transport Id'},
  ...
  public onEdit(event) {
    ...
    sessionStorage.setItem('transportData', event.ModeOfTransportId);
    this.router.navigateByUrl('Marine/masters/transport/add-edit');
  }
}
