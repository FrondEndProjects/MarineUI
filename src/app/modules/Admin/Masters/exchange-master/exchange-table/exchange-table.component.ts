...
export class ExchangeTableComponent implements OnInit {
     ...
     this.masterSer.onPostMethodSync(`${this.ApiUrl1}master/exchange/list`, ReqObj).subscribe(
       (data: any) => {
         if (data?.Message === 'Success') {
           console.log(data);
          this.columnHeader = [
            {key: 'ExchangeId', display: 'Exchange ID'},
 ...
 public onEdit(event) {
   ...
   sessionStorage.setItem('exchangeData', event.ExchangeId);
   this.router.navigateByUrl('Marine/masters/exchange-master/add-edit');
 }
