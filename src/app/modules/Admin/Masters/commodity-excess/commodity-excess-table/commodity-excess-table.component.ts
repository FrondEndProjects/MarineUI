...
export class CommodityExcessTableComponent implements OnInit {
     ...
     this.masterSer.onPostMethodSync(`${this.ApiUrl1}master/commodityexcess/list`, ReqObj).subscribe(
       (data: any) => {
         if (data?.Message === 'Success') {
           console.log(data);
          this.columnHeader = [
            {key: 'Sno', display: 'Complex ID'},
            {key: 'StartSuminsured', display: 'Start Amount'},
            {key: 'EndSuminsured', display: 'End Amount'},
            {key: 'Deductible', display: 'Deductible'},
 ...
 public onEdit(event) {
   ...
   sessionStorage.setItem('commodityData', event.Sno);
   this.router.navigateByUrl('Marine/masters/commodity-excess/add-edit');
 }
