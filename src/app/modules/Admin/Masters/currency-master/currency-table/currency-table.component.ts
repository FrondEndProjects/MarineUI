...
// constructor
  ...
  this.masterSer.onPostMethodSync(`${this.ApiUrl1}master/currency/list`, ReqObj).subscribe(
    (data: any) => {
      ...
      if(data?.Message == 'Success') {
        this.columnHeader = [
          {key: 'CurrencyId', display: 'Currency ID'},
          {key: 'CurrencyName', display: 'Currency Type'},
          {key: 'SubCurrency', display: 'Sub Currency'},
          {key: 'CurrencyShortName', display: 'Short Name'},
 ...
 public onEdit(event) {
   sessionStorage.setItem('currencyData', event.CurrencyId);
   this.router.navigateByUrl('Marine/masters/currency/add-edit');
 }
